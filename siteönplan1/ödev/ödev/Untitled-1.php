...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["email"]))) {
        ...
        if(mysqli_num_rows($sonuc) == 0){
            ...
            $mail = new PHPMailer();
            $mail->isSMTP();
            $mail->Host = 'smtp.gmail.com'; // Düzeltildi
            $mail->SMTPAuth = true;
            $mail->Username = 'eekinci871@gmail.com';
            $mail->Password = 'nfkg cllb npao jzdh';
            $mail->SMTPSecure = 'tls'; // Düzeltildi
            $mail->Port = 587; // Düzeltildi
            $mail->CharSet  ="UTF-8";
            $mail->SMTPDebug = 2; // Hata ayıklama modu
            ...
            $sifirlamalinki="http://localhost/panel/sifreolustur.php?kod=".$sifirlamakodu;
            $mail->Body = $sifirlamalinki;

            if($mail->send()) {
                echo '<div class="alert alert-success text-center">Yeni Şifre için E-posta Gönderilmiştir.</div>';
            } else {
                echo '<div class="alert alert-danger text-center">E-posta gönderilirken bir hata oluştu: ' . $mail->ErrorInfo . '</div>';
            }
        }
    }
}
