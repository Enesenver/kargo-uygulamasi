<?php
    ...
    $sonuc1 = mysqli_query($baglan, "SELECT * FROM `kullanicii` WHERE `sifirlamakodu`='$kod'");
                     
    if(mysqli_num_rows($sonuc1) == 0){
        header("refresh:5 ; url=sifremiunuttum.php"); 
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        if(empty($sifre_hata) && empty($sifre_onay_hata)) {
            ...
            $sorgu= "UPDATE `kullanici_bilgileri` SET `sifre`='$sifre' WHERE `sifirlamakodu`='$kod'";
