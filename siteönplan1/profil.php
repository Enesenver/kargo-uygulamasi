...
if(session_bilgi() == true){
    ...
    $ilce = isset($row["ilce"]) ? $row["ilce"] : "";
    $il = isset($row["il"]) ? $row["il"] : "";
}
...
if(!empty($_POST["kullaniciadi"])){
    ...
    if($kullaniciadi == $kullaniciadi_veritabani){
        ...
        if(empty($kullaniciadi_hata)){
            $kullaniciadi = kullaniciadi_guncelle($kullaniciadi);
...
if(isset($_POST["sifre_degistir"])){
    ...
    if(empty($eski_sifre)){
        $eski_sifre_hata = "Mevcut şifrenizi girdiniz.";
    } else {
    ...
    if(empty($yeni_sifre)){
        $yeni_sifre_hata = "Lütfen yeni şifrenizi girdiniz.";
    } else {
    ...
    if(empty($yeni_sifre)){
        $yeni_sifre_onay_hata = "Lütfen yeni şifrenizi tekrar girdiniz.";
    } else {
...
if(isset($_POST["kayit"])){
    $sonuc = bilgi_guncelle($_POST["ad"], $_POST["soyad"], $_POST["email"], $_POST["adres"], $_POST["ilce"], $_POST["il"]);
    if ($sonuc) {
        echo '<div class="alert alert-success text-center m-2 p-2">Bilgileriniz güncellenmiştir.</div>';
    } else {
        echo '<div class="alert alert-danger text-center m-2 p-2">Bilgileriniz güncellenemedi. Lütfen tekrar deneyin.</div>';
    }
}
?>
